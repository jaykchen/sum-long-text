...
use llm_chain::text_splitter::{NaiveWhitespaceSplitter, TextSplitter};// use llm_chain_openai::executor;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let artcle_parts = splitter.split_text(article, 200, 20);

if let Ok(chunks) = artcle_parts {
    let docs = chunks
        .into_iter()
        .map(|part| parameters!(part))
        .collect::<Vec<_>>();
